...
public class Dijkstra {
    ...
    public Dijkstra(Board board) {
        ...
        this.idGold = 0;
        this.idWumpus = 0;
        this.proxi = new boolean[boardWidth][boardHeight][boardWidth][boardHeight];
            ...
            for (int j = 1; j < tabContent.length - 1; j++) {
                ...
                for (Map.Entry<Direction, Case> aroundEntry : caseAround.entrySet()) {
                    ...
                    if (value.containsContent(Content.PIT)) {
                        ...
                        proxi[i][j][value.getX()][value.getY()] = false;
...
//                        idWumpus = value.getId();
                        proxi[i][j][value.getX()][value.getY()] = true;
...
//        g.printResult(idGold, board, xGold, yGold, tabContent);
        ...
        shortestPath.path();
        board.getCase(1, 1).addContent(Content.DIJKSTRA);
        int firstElement;
        int secondElement;
        for (int[] elementPath : shortestPath.path) {
            firstElement = elementPath[0];
            secondElement = elementPath[1];
            board.getCase(firstElement, secondElement).addContent(Content.DIJKSTRA);
        }
    }
}